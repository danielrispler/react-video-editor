...
export const getFfprobePath = (): string => ffmpegInstaller.path.replace('ffmpeg', 'ffprobe');

ffmpeg.setFfprobePath(getFfprobePath());

export const runFfmpeg = (builder: (command: ReturnType<typeof ffmpeg>) => ReturnType<typeof ffmpeg>, timeoutMs: number = 0): Promise<void> => {
  return new Promise((resolve, reject) => {
    const command = builder(ffmpeg());

    const state = { isTimedOut: false };
    const timeoutHandle: NodeJS.Timeout | null = timeoutMs > 0
      ? setTimeout(() => {
...
const doesStreamHaveAudio = (stream: ffmpeg.FfprobeStream): boolean => stream.codec_type !== null && stream.codec_type === 'audio';
...
export const hasAudioStream = (videoPath: string): Promise<boolean> =>
  new Promise((resolve) => {
    ffmpeg.ffprobe(videoPath, (err, metadata) => {
      ...
      resolve(!hasAudio || err ? false : true);
    });
  });
