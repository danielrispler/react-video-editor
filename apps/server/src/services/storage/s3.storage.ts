...
export class S3Storage implements StorageProvider {
    ...
    public uploadStream = async (stream: Readable, key: string, contentType?: string): Promise<void> => {
    ...
    public downloadToFile = async (urlOrKey: string, outputPath: string): Promise<void> => this.downloadFile(this.extractKeyFromUrlOrKey(urlOrKey), outputPath);

    public getPresignedUrl = async (key: string, expiresIn: number = 3600): Promise<string> => {
        const command = new GetObjectCommand({
            Bucket: this.bucket,
            Key: key
        });
    ...
    public getPresignedUploadUrl = async (key: string, contentType?: string, expiresIn: number = 3600): Promise<string> => {
