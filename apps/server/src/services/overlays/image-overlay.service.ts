...
export const prepareImageOverlays = async (overlays: Overlay[], tempDir: string): Promise<{ imageOverlayPaths: string[]; hasOverlays: boolean }> => {
    ...
    for (const overlay of overlays) {
        if (overlay.type === OverlayType.image) {
            ...
            const finalImagePath: string = originalExt === 'webp'
                ? await convertWebpToPng(imagePath)
                : imagePath;
...
export const buildImageOverlayFilter = (overlay: ImageOverlay, imageInputIndex: number, currentStream: string, outputLabel: string, videoDuration: number): string => {
