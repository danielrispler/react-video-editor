...
export function getFontFileParameter(): string {
    ...
    if (!fontPath) {
        ...
        throw new Error(
            `No font filename provided: no usable font found for drawtext. ` +
            `Tried (${platform}): ${candidates}. ` +
            `Set FFMPEG_FONT_PATH to a path to a .ttf font file, or install one of the candidate fonts.`
        );
    }
