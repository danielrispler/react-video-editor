...
export default (str: string) => {
	...
	const defaultStops = {
		...
		gradient:
			"linear-gradient(180deg, rgba(6, 6, 6, 1) 0.0%, rgba(183, 80, 174, 0.92) 100.0%)",
		modifier: 180,
		type: "linear",
	};
	...
	const normalizedGradient = str
		.replace(";", "")
		.replace("background-image:", "");
	const gradient = validGradient(normalizedGradient);
	...
	const [, type, content] =
		normalizedGradient.match(/^(\w+)-gradient\((.*)\)$/i) || [];
	if (!type || !content) {
		console.warn("Incorrect gradient value");
		return defaultStops;
	}
	...
	return {
		gradient: `${type}-gradient(${
			typeof gradient !== "string" ? gradient.original : normalizedGradient
		})`,
		type,
		modifier:
