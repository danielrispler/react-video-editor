		...
		while (matchColorStop !== null) {
			...
			if (matchColorStop[2]) {
				stopResult.position = Number(
					(Number.parseInt(matchColorStop[2], 10) / 100).toFixed(2),
				);
			}
...
export default (input: string) => {
	...
	if (match !== null) {
		...
		if (
			result.stops.every((item) => Object.hasOwn(item, "position")) === false
		) {
