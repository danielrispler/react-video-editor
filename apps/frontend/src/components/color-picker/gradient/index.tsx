...
const Gradient: FC<IPropsComp> = ({
	...
	const fallbackStop = stops[stops.length - 1];
	const lastStop = fallbackStop
		? (rgbaToArray(fallbackStop[0]) ?? [0, 0, 0, 1])
		: [0, 0, 0, 1];
	const lastStopLoc = fallbackStop?.[1] ?? 0;
	const activeStop = rgbaToHex([lastStop[0], lastStop[1], lastStop[2]]);
	const activeIdx = fallbackStop?.[2] ?? 0;
	...
	const onSubmitChange = (rgba: string) => {
		const rgbaArr = rgbaToArray(rgba) ?? [0, 0, 0, 1];
