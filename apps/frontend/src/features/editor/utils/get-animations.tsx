...
const getSlideAnimation = (
	...
	if (type === "slideInRight" || type === "slideOutLeft") {
		const commonValue =
			-Number.parseFloat(item.details.left) - item.details.width / scale;
	...
	if (type === "slideInLeft" || type === "slideOutRight") {
		const commonValue =
			Number.parseFloat(item.details.left) + item.details.width / scale;
		const from = type.includes("In") ? commonValue : anim.from;
		const to = type.includes("In") ? anim.to : commonValue;
	...
	if (type === "slideInBottom" || type === "slideOutTop") {
		const commonValue =
			-Number.parseFloat(item.details.top) - item.details.height / scale;
		const from = type.includes("In") ? commonValue : anim.from;
		const to = type.includes("In") ? anim.to : commonValue;
		return {
			property: anim.property,
	...
	if (type === "slideInTop" || type === "slideOutBottom") {
		const commonValue =
			Number.parseFloat(item.details.top) + item.details.height / scale;
		const from = type.includes("In") ? commonValue : anim.from;
		const to = type.includes("In") ? anim.to : commonValue;
