...
function scaleDiv(
	...
	if (div) {
		const fontSize = Number.parseFloat(getComputedStyle(div).fontSize);
			...
			onResize={({
				...
				if (direction[1] === 1 || direction[1] === -1) {
					if (trackItemsMap[id].type === "progressSquare") {
						const diffWidth =
							nextHeight - Number.parseFloat(target.style.height);
					...
					if (trackItemsMap[id].type === "progressSquare") {
						...
						if (direction[0] === -1) {
							...
							target.style.left = `${
								Number.parseFloat(target.style.left) - diffWidth
							}px`;
							updateData.left = `${
								Number.parseFloat(target.style.left) - diffWidth
							}px`;
